...
//console.log(getNumberOfMovies());


function NumberOfMoviesPerEachYear(){
    const result =data.reduce((acc,curr)=>{
        let Year =curr.Year;
        let Genre=curr.Genre;
...
//console.log(NumberOfMoviesPerEachYear());


function averagePerGenre(){
    const filteredData = data.filter(movie => !isNaN(movie.Gross_Earning_in_Mil) && movie.Gross_Earning_in_Mil !== "NA");
    
    const result = filteredData.reduce((acc, curr) => {
        let Genre = curr.Genre;
        let Gross_Earning_in_Mil = parseFloat(curr.Gross_Earning_in_Mil);
        
        if(acc[Genre]){
            acc[Genre].earnings += Gross_Earning_in_Mil;
    ...
    const averageScore = Object.entries(result).reduce((result, [genre, details]) => {
        result[genre] = parseFloat((details.earnings / details.genreCount).toFixed(2));
        return result;
    }, {});
...
//console.log(averagePerGenre());


function averageEarning() {
    try {
      const result = data.reduce(function (accum, element) {
        const year = element.Year;
        const genre = element.Genre;
        const earning=element.Gross_Earning_in_Mil;
  
        if (accum[year]) {
  
          accum[year].count++;
          accum[year].earn+=earning;
        } else {
          accum[year] = { gen: genre, count: 1 , earn: earning};
        }
        return accum;
      }, {});
      ...
      Object.entries(result).forEach(function(element){
          let data=element[1];
          data["avgEarning"]=data.earn/data.count;
          // console.log(data);
      });
      console.log(result);
    } catch (error) {
